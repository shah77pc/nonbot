...
x_data = get_features_v2(oanda_data, time_periods=[
                         10, 25, 50, 120, 256], return_numpy=False)
...
input_train, input_test, input_cv = get_poloynomials(
    input_train, input_test, input_cv, degree=2)
...
for _ in range(5000):
    ...
    if step % 10 == 0 or step == 1:
        ...
        value_train = portfolio_value(
            price_train, signal_train, trans_costs=transaction_c)
        value_test = portfolio_value(
            price_test, signal_test, trans_costs=transaction_c)
        value_cv = portfolio_value(
            price_cv, signal_cv, trans_costs=transaction_c)
        ...
        if plotting:
            ...
            plt.plot(step_hist, cost_hist_train,
                     color='darkorange', linewidth=0.3)
            plt.plot(step_hist, cost_hist_test,
                     color='dodgerblue', linewidth=0.3)

            plt.subplot(212)
            plt.title('Portfolio value')
            plt.plot(step_hist, value_hist_train,
                     color='darkorange', linewidth=0.3)
            plt.plot(step_hist, value_hist_test,
                     color='dodgerblue', linewidth=0.3)
            plt.plot(step_hist, value_hist_cv, color='magenta', linewidth=1)
            plt.plot(step_hist, value_hist_train_ma,
                     color='tomato', linewidth=1.5)
            plt.plot(step_hist, value_hist_test_ma,
        ...
        if saving:
            ...
            if saving_score == current_score and saving_score > 0.1:
                saver.save(
                    sess, 'saved_models/lr-v1-avg_score{:.3f}'.format(current_score), global_step=step)
