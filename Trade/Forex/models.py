...
def logistic_regression(input_dim, output_dim):
    ...
    w = tf.get_variable('weights1', shape=[
                        input_dim, output_dim], initializer=w_init)
...
def lstm_nn(input_dim, output_dim, time_steps, n_hidden):
    ...
    w_init = tf.contrib.layers.xavier_initializer()
    b_init = tf.initializers.truncated_normal(mean=0.1, stddev=0.025)
    w = tf.get_variable('last_weights', shape=[
                        n_hidden[-1], output_dim], initializer=w_init)
    # b = tf.get_variable('bias1', shape=[output_dim], initializer=b_init)
    ...
    for layer_size in n_hidden:
        stacked_lstm_cells.append(tf.contrib.rnn.BasicLSTMCell(
            layer_size, activation=tf.nn.relu))

    # create the net and add dropout
    lstm_cell = tf.contrib.rnn.MultiRNNCell(stacked_lstm_cells)
    lstm_cell_with_dropout = tf.contrib.rnn.DropoutWrapper(
        lstm_cell, output_keep_prob=drop_out)

    # forward propagate
    outputs, state = tf.contrib.rnn.static_rnn(
        lstm_cell_with_dropout, x_split, dtype=tf.float32)
    # + b  # logits are used for cross entropy
...
def vanilla_nn(input_dim, output_dim, architecture, drop_layer=0, drop_keep_prob=0.9):
    ...
    for layer, size_current, size_next in zip(range(len(layer_sizes)), layer_sizes, layer_sizes[1:]):
        ...
        # create weights
        # dummy variable for last layer
        last_layer = layer == len(layer_sizes) - 2
        weights[layer] = tf.get_variable('weights{}'.format(layer), shape=[
                                         size_current, size_next], initializer=w_init)
        biases[layer] = tf.get_variable('biases{}'.format(
            layer), shape=[size_next], initializer=b_init)

        # forward-propagate
        if not last_layer:
            layer_values[layer+1] = tf.nn.relu(
                tf.matmul(layer_values[layer], weights[layer]) + biases[layer])
        else:
            layer_values[layer+1] = tf.matmul(layer_values[layer],
                                              weights[layer]) + biases[layer]
            y_ = tf.nn.softmax(layer_values[layer+1])
        if drop_layer == layer:
            layer_values[layer+1] = tf.nn.dropout(
                layer_values[layer+1], keep_prob=drop_keep_prob)
